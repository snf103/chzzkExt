...
(async function () {
  ...
  const initVote = async () => {
    ...
    const getList = () => {
      return [
        ...document.querySelectorAll(
          "#root > div > section > aside > div > div"
        ),
      ]
        .map((x) => [x, x.className])
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        ...
        const ucolor =
          c.children[0].children[c.children[0].childElementCount - 1].style
            .color;
  ...
  const initVoteOpenButton = async () => {
    ...
    myBtn.addEventListener("mouseenter", () => {
      ...
      myBtn.children[0].style.backgroundColor = "var(--color-bg-overlay-03)";
      myBtn.children[0].style.borderRadius = "6px";
      myBtn.children[0].style.padding = "2px";
      myBtn.children[0].style.margin = "-2px";
    });
    myBtn.addEventListener("mouseleave", () => {
      myBtn.style.color = "var(--color-content-04)";
      myBtn.children[0].style.backgroundColor = "transparent";
      myBtn.children[0].style.borderRadius = "0px";
      myBtn.children[0].style.padding = "0px";
      myBtn.children[0].style.margin = "0px";
    });
  };
  ...
  const apply = () => {
    ...
    console.log("apply", window.location.href);
    if (window.chzzkExt.lastAppliedURL != window.location.href)
