...
export default function initUI_ED() {
  ...
  const interval = () => {
    ...
    onoffer(
      document.querySelector(
        `[href="https://game.naver.com/profile#cheat_key"]`
      ),
      cfg("ed_chkey")
    );
  ...
  if (typeof window.chzzkExt.uied != "undefined") {
    clearInterval(window.chzzkExt.uied);
  }

  window.chzzkExt.uied = setInterval(interval, 1000);
}
